...
async function dump() {
    ...
    for(let pageNum=1; pageNum<=pdf.numPages; pageNum++) {
        ...
        for(let i=0; i<Math.min(90, textContent.items.length); i++) {
           const item = textContent.items[i];
           if (item.str.trim()) {
               console.log(`Page ${pageNum}: "${item.str}" at (${item.transform[4].toFixed(2)}, ${item.transform[5].toFixed(2)}) w: ${item.width.toFixed(2)}`);
           }
        }
    }
}
