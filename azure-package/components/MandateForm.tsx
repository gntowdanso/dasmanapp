...
export default function MandateForm({ 
  ...
  const resizeSignatureCanvas = useCallback(() => {
    ...
    // Only resize if actual dimensions have changed to prevent mobile scroll-wiping
    if (
    ...
    // Use Math.floor to avoid floating point mismatch on subsequent checks
    canvas.width = Math.floor(width * ratio);
    canvas.height = Math.floor(height * ratio);
    ...
    if (dataUrl) {
      // Small timeout to allow canvas setup before drawing
      setTimeout(() => {
